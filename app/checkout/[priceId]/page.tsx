...
export default function CheckoutPage() {
  ...
  const displayPrice = price ? `$${price}` : checkoutData?.totals.total ? `$${(checkoutData.totals.total / 100).toFixed(2)}` : 'Loading...';
  const displayCycle = cycle ? cycle.charAt(0).toUpperCase() + cycle.slice(1) : '';
    ...
    if (!paddle?.Initialized && 
      ...
      initializePaddle({
        ...
        toast({
          ...
          description: 'Failed to initialize checkout. Please refresh and try again.',
          variant: 'destructive',
        });
        initializationRef.current = false;
      });
    }
  }, [paddle?.Initialized, priceId, user?.email, isValidPrice, handleCheckoutEvents]);
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-semibold text-lg">{displayPlan}</h3>
                        {displayCycle && (
                  ...
                  <div className="bg-muted p-3 rounded-lg">
                    <p className="text-sm text-muted-foreground">
                      ...
                      Secure checkout powered by Paddle. Your payment information is encrypted and secure.
                    </p>
                  </div>
                </CardContent>
              </Card>
