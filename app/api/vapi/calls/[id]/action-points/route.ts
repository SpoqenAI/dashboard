...
function extractActionPointsFromVapi(callData: any): ActionPoints {
  ...
  if (structured.keyPoints && Array.isArray(structured.keyPoints)) {
    keyPoints.push(...structured.keyPoints.filter((point: any) => point && point.trim().length > 0));
  }
  ...
  if (structured.followUpItems && Array.isArray(structured.followUpItems)) {
    followUpItems.push(...structured.followUpItems.filter((item: any) => item && item.trim().length > 0));
  }
  
  // Also check for legacy field names
  if (structured.actionItems && Array.isArray(structured.actionItems)) {
    followUpItems.push(...structured.actionItems.filter((item: any) => item && item.trim().length > 0));
  }
  
  if (structured.followUps && Array.isArray(structured.followUps)) {
    followUpItems.push(...structured.followUps.filter((item: any) => item && item.trim().length > 0));
  }
  
  // 2. Fallback: Extract actionable items from text
  if (followUpItems.length === 0) {
    const extractedActions = extractFollowUpItemsFromText(summary || transcript);
  ...
  if (structured.urgentConcerns && Array.isArray(structured.urgentConcerns)) {
    urgentConcerns.push(...structured.urgentConcerns.filter((concern: any) => concern && concern.trim().length > 0));
  }
  
  // Also check for legacy field names
  if (structured.urgentItems && Array.isArray(structured.urgentItems)) {
    urgentConcerns.push(...structured.urgentItems.filter((concern: any) => concern && concern.trim().length > 0));
  }
  
  // 2. Fallback: Extract urgent indicators from text
  if (urgentConcerns.length === 0) {
    const extractedUrgent = extractUrgentConcernsFromText(summary || transcript);
  ...
  else if (callData.metadata?.purpose && callData.metadata.purpose.trim().length > 0) {
  ...
  if (structured.sentiment && ['positive', 'neutral', 'negative'].includes(structured.sentiment.toLowerCase())) {
    sentiment = structured.sentiment.toLowerCase() as 'positive' | 'neutral' | 'negative';
  }
  // 2. Secondary: Use analysis-level sentiment
  else if (analysis?.sentiment && ['positive', 'neutral', 'negative'].includes(analysis.sentiment.toLowerCase())) {
...
function extractCallPurposeFromText(text: string): string {
  ...
  const purposePatterns = [
    { pattern: /(?:buying|purchase|looking for|interested in buying)\s+(?:a\s+)?(?:house|home|property)/i, purpose: 'Home buying inquiry' },
    { pattern: /(?:selling|list|putting.+on.+market)\s+(?:my\s+)?(?:house|home|property)/i, purpose: 'Home selling consultation' },
    { pattern: /(?:rent|rental|lease|tenant)/i, purpose: 'Rental inquiry' },
    { pattern: /(?:investment|investor|investing)\s+(?:property|real estate)/i, purpose: 'Investment opportunity' },
    { pattern: /(?:valuation|appraisal|worth|value)\s+(?:of\s+)?(?:my\s+)?(?:house|home|property)/i, purpose: 'Property valuation' },
    { pattern: /(?:mortgage|financing|loan|pre.?approval)/i, purpose: 'Financing consultation' },
    { pattern: /(?:showing|tour|visit|see|view)\s+(?:the\s+)?(?:house|home|property)/i, purpose: 'Property showing request' },
    { pattern: /(?:offer|bid|making.+offer)/i, purpose: 'Offer discussion' },
    { pattern: /(?:market|area|neighborhood)\s+(?:analysis|report|update)/i, purpose: 'Market consultation' },
  ];
 ...
 */
function deriveSentimentFromSuccess(successEvaluation: any): 'positive' | 'neutral' | 'negative' {
  ...
  if (typeof successEvaluation === 'string') {
    ...
    if (lower.includes('fail') || lower.includes('negative') || lower.includes('bad')) {
      return 'negative';
    }
  }
 ...
 */
function analyzeSentimentFromText(text: string): 'positive' | 'neutral' | 'negative' {
  ...
  // Positive indicators
  const positiveWords = [
    'great', 'excellent', 'perfect', 'love', 'interested', 'excited', 
    'happy', 'satisfied', 'good', 'wonderful', 'amazing', 'fantastic',
    'yes', 'definitely', 'absolutely', 'sounds good'
  ];
  
  // Negative indicators
  const negativeWords = [
    'bad', 'terrible', 'awful', 'hate', 'disappointed', 'frustrated',
    'angry', 'upset', 'no', 'not interested', 'waste', 'problem',
    'issue', 'concern', 'worried', 'expensive', 'too much'
  ];
