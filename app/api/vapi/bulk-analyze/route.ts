...
export async function POST(request: NextRequest) {
  try {
    ...
    const answeredCalls = userCalls.filter((call: any) => {
      ...
      const hasVapiAnalysis =
        call.analysis &&
        (call.analysis.summary || call.analysis.structuredData);
      const hasContent =
        hasTranscript || hasSummary || hasMessages || hasVapiAnalysis;
          ...
          try {
            ...
            if (detailResponse.ok) {
              ...
              logger.info('BULK_ANALYZE', 'Fetched detailed call info', {
                ...
                hasDetailedStructuredData:
                  !!detailedCall.analysis?.structuredData,
...
function extractAnalysisFromVapiCall(call: any) {
  ...
  keyPoints = Array.isArray(keyPoints)
    ...
    : keyPoints
      ? [keyPoints]
      : ['Standard inquiry'];
  followUpItems = Array.isArray(followUpItems)
    ? followUpItems
    : followUpItems
      ? [followUpItems]
      : [];
  urgentConcerns = Array.isArray(urgentConcerns)
    ? urgentConcerns
    : urgentConcerns
      ? [urgentConcerns]
      : [];
  leadQuality =
    leadQuality ||
    determineLeadQualityFromContent(summary || fallbackContent, urgentConcerns);
  timeline = timeline || extractTimelineFromText(summary || fallbackContent);
  contactPreference =
    contactPreference ||
    extractContactPreferenceFromText(summary || fallbackContent);
  ...
  businessInterest =
    businessInterest ||
    extractBusinessInterestFromText(summary || fallbackContent);
...
function extractBusinessInterestFromText(content: string): string {
  ...
  if (lower.includes('crm') || lower.includes('customer management'))
    return 'CRM/Customer Management';
  if (
...
function determineLeadQualityFromContent(
  content: string,
