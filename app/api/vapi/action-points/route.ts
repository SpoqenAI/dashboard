...
export async function POST(request: NextRequest) {
  try {
    ...
    const vapiResponse = await fetch(`${process.env.VAPI_API_URL || 'https://api.vapi.ai'}/call/${callId}`, {
      headers: {
    ...
    if (!vapiResponse.ok) {
      logger.error('ACTION_POINTS', 'Failed to fetch call details from VAPI', undefined, {
        callId,
    ...
    const transcript = callData.transcript || callData.messages?.map((m: any) => `${m.role}: ${m.message}`).join('\n');
    ...
    try {
      ...
      if (userError || !user) {
        logger.error('ACTION_POINTS', 'Failed to get authenticated user', userError || new Error('No authenticated user'));
      } else {
        ...
        if (insertError) {
          logger.error('ACTION_POINTS', 'Failed to save call analysis to database', insertError);
        } else {
          logger.info('ACTION_POINTS', 'Successfully saved call analysis to database', { callId });
        }
      }
    } catch (dbError) {
      logger.error('ACTION_POINTS', 'Database operation failed', dbError as Error);
    }
    ...
    logger.error('ACTION_POINTS', 'Error generating action points', error as Error);
    return NextResponse.json(
...
async function generateActionPointsWithAI(content: string, callData: any): Promise<ActionPoints> {
  ...
  const positiveWords = ['great', 'excellent', 'interested', 'perfect', 'love', 'amazing', 'wonderful'];
  const negativeWords = ['terrible', 'awful', 'hate', 'disappointed', 'problem', 'issue', 'wrong'];
  ...
  if (lowerContent.includes('appointment') || lowerContent.includes('showing') || lowerContent.includes('visit')) {
    followUpItems.push('Schedule property showing or appointment');
  }

  if (lowerContent.includes('budget') || lowerContent.includes('price') || lowerContent.includes('afford')) {
    keyPoints.push('Budget and pricing discussed');
  }

  if (lowerContent.includes('timeline') || lowerContent.includes('when') || lowerContent.includes('soon')) {
  ...
  if (lowerContent.includes('competitor') || lowerContent.includes('other agent')) {
  ...
  return {
    callPurpose,
    sentiment,
    keyPoints: keyPoints.length > 0 ? keyPoints : ['Standard real estate inquiry'],
    followUpItems,
    urgentConcerns,
    callAnalysis: {
      leadQuality: urgentConcerns.length > 0 ? 'hot' : keyPoints.length > 2 ? 'warm' : 'cold',
      appointmentRequested: lowerContent.includes('appointment') || lowerContent.includes('showing'),
...
function extractPropertyType(content: string): string {
  if (content.includes('condo') || content.includes('condominium')) return 'Condominium';
  if (content.includes('house') || content.includes('home')) return 'Single Family Home';
  if (content.includes('townhouse') || content.includes('townhome')) return 'Townhouse';
...
function extractTimeline(content: string): string {
  if (content.includes('immediately') || content.includes('asap') || content.includes('urgent')) return 'Immediate';
  if (content.includes('month') || content.includes('30 days')) return 'Within 1 month';
  if (content.includes('months') || content.includes('quarter')) return 'Within 3 months';
