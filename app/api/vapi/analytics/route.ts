...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logger.info('ANALYTICS', 'Filtering calls by user assistant and date range', {
      daysFilter: days,
    ...
    if (userFilteredCalls.length > 0) {
      ...
      const allStatuses = [...new Set(userFilteredCalls.map(call => call.status).filter(Boolean))];
      const allEndedReasons = [...new Set(userFilteredCalls.map(call => call.endedReason).filter(Boolean))];
    ...
    const metrics = await calculateMetrics(mappedUserCalls, user?.id, supabase, days);
    ...
    if (userFilteredCalls.length > 0) {
      const callDates = userFilteredCalls.map(call => call.createdAt || call.startedAt);
      const oldestCall = new Date(Math.min(...callDates.map(d => new Date(d).getTime())));
      const newestCall = new Date(Math.max(...callDates.map(d => new Date(d).getTime())));
      ...
      logger.info('ANALYTICS', 'Call date range verification', {
        ...
        daysSinceOldest: Math.floor((Date.now() - oldestCall.getTime()) / (1000 * 60 * 60 * 24)),
      });
    }
...
async function calculateMetrics(calls: any[], userId?: string, supabase?: any, days?: number): Promise<CallMetrics> {
  ...
  const answeredCallsData = calls.filter(call => {
    ...
    if (call.analysis?.successEvaluation !== undefined) {
      ...
      if (typeof success === 'boolean') {
        ...
        if (successLower === 'true' || successLower === 'pass' || successLower === 'successful') {
          result = true;
          evaluationPath += '-> SUCCESS_BY_AI_ANALYSIS ';
        } else if (successLower === 'false' || successLower === 'fail' || successLower === 'failed') {
          result = false;
        ...
        if (success >= 7) { // Assuming 1-10 scale, 7+ is success
          result = true;
          evaluationPath += '-> SUCCESS_BY_AI_ANALYSIS ';
        } else {
          result = false;
      ...
      if (evaluationPath.includes('SUCCESS_BY_AI_ANALYSIS') || evaluationPath.includes('FAILED_BY_AI_ANALYSIS')) {
        if (result === false) {
    ...
    if (call.endedReason) {
      ...
      if (['silence-timed-out', 'silence timed out', 'assistant-error', 'no-answer', 
      ...
      if (['customer-ended-call', 'assistant-ended-call'].includes(endedReason)) {
        result = true;
    ...
    if (call.status) {
      ...
      if (['failed', 'busy', 'no-answer', 'cancelled', 'timeout'].includes(status)) {
        result = false;
    ...
    if (duration >= 5) { // Require at least 5 seconds for success
      result = true;
  ...
  const totalDuration = answeredCallsData.reduce((sum, call) => sum + (call.durationSeconds || 0), 0);
  ...
  const analysisBreakdown = {
    ...
    callsWithAIAnalysis: calls.filter(call => call.analysis?.successEvaluation !== undefined).length,
    aiAnalysisSuccessful: calls.filter(call => {
      ...
      if (typeof success === 'string') {
        const successLower = success.toLowerCase();
        return successLower === 'true' || successLower === 'pass' || successLower === 'successful';
      }
    ...
    successByEndedReason: calls.filter(call => {
      ...
      const endedReason = call.endedReason.toLowerCase();
      return ['customer-ended-call', 'assistant-ended-call'].includes(endedReason);
    }).length,
    failedByEndedReason: calls.filter(call => {
      if (!call.endedReason) return false;
      const endedReason = call.endedReason.toLowerCase();
      return ['silence-timed-out', 'silence timed out', 'assistant-error', 'no-answer', 
              'customer-did-not-give-microphone-permission'].includes(endedReason);
    }).length,
    ...
    explicitFailureStatuses: calls.filter(call => {
      if (!call.status) return false;
      const status = call.status.toLowerCase();
      return ['failed', 'busy', 'no-answer', 'cancelled', 'timeout'].includes(status);
    }).length,
    ...
    durationFallbacks: calls.filter(call => {
      ...
      const hasEndedReason = call.endedReason && (
        ['customer-ended-call', 'assistant-ended-call', 'silence-timed-out', 'silence timed out', 
         'assistant-error', 'no-answer', 'customer-did-not-give-microphone-permission'].includes(
          call.endedReason.toLowerCase()
        )
      );
      const hasExplicitStatus = call.status && (
        ['completed', 'answered', 'failed', 'busy', 'no-answer', 'cancelled', 'timeout'].includes(
  ...
  logger.info('ANALYTICS', 'Call success evaluation breakdown', analysisBreakdown);
...
function calculateTrends(calls: any[], days: number): DashboardAnalytics['trends'] {
  ...
  const recentAvgDuration = recentAnsweredCalls.length > 0 
    ? recentAnsweredCalls.reduce((sum, call) => sum + (call.durationSeconds || 0), 0) / recentAnsweredCalls.length
    : 0;
  const olderAvgDuration = olderAnsweredCalls.length > 0
    ? olderAnsweredCalls.reduce((sum, call) => sum + (call.durationSeconds || 0), 0) / olderAnsweredCalls.length
    : 0;
