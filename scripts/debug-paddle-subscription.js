  ...
  try {
    ...
    if (subError) {
      ...
      subscriptions.forEach((sub, index) => {
        ...
        console.log(
          `     Current period: ${sub.current_period_start_at} to ${sub.current_period_end_at}`
        );
    ...
    if (latestError) {
      ...
      console.log(
        '🎯 Latest subscription (as used by useSubscription hook):',
        latestSub
          ? {
              id: latestSub.id,
...
async function listAllSubscriptions() {
  ...
  data.forEach((sub, index) => {
    console.log(
      `${index + 1}. ${sub.profiles.email} (${sub.profiles.first_name} ${sub.profiles.last_name})`
    );
...
if (command === 'list') {
  ...
  console.log('Usage:');
  console.log(
    '  node scripts/debug-paddle-subscription.js [user_id] - Debug specific user'
  );
  console.log(
    '  node scripts/debug-paddle-subscription.js list - List all subscriptions'
  );
  console.log('');
  console.log('Example:');
  console.log(
    '  node scripts/debug-paddle-subscription.js cf1805db-401e-4cac-8a54-0f4ea54eafa3'
  );
}
