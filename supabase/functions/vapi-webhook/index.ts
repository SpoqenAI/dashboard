...
serve(async (req)=>{
  /* 1. Secret header check --------------------------------------------- */ const incomingSecret = req.headers.get('x-vapi-secret') ?? '';
  ...
  const valid = incomingBytes.length === expectedBytes.length && timingSafeEqual(incomingBytes, expectedBytes);
  if (!valid) {
    return new Response('Unauthorized', {
      status: 401
    });
  }
  /* 2. Parse message ---------------------------------------------------- */ let envelope;
  try {
  ...
  if (msg.type !== 'end-of-call-report') return new Response(null, {
    status: 200
  });
  ...
  /* 3. Invoke mail-sending function ------------------------------------ */ const summary = msg.summary ?? msg.analysis?.summary ?? '';
  try {
    const { error: invokeErr } = await supabase.functions.invoke('send-email-summary', {
      body: {
        assistantId,
        summary,
    ...
    if (invokeErr) {
      ...
      return new Response('Email function failed', {
        status: 502
      });
    }
  } catch (err) {
    console.error('Error invoking send-email-summary', err);
    return new Response('Email function error', {
      status: 500
    });
  }
