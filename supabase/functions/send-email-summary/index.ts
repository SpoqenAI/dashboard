...
import { initSentry, captureException, addBreadcrumb, setUser, setTag, startTransaction } from '../_shared/sentry.ts';
...
serve(async (req)=>{
  ...
  try {
    ...
    const { assistantId, summary, phoneNumber, analysis } = payload;
    ...
    if ((typeof summary !== 'string' || summary.trim().length === 0) && (typeof analysis !== 'object' || analysis === null)) {
      const error = new Error('Invalid summary or analysis');
      captureException(error, {
        function: 'send-email-summary',
      ...
      return new Response('Invalid summary or analysis', {
        status: 400
      });
    }
    ...
    /* a. Resolve user & email preferences -------------------------------- */ addBreadcrumb('Fetching user settings', 'database', {
      assistant_id: assistantId
    });
    const { data: userSettings } = await supabase.from('user_settings').select('id,email_notifications').eq('vapi_assistant_id', assistantId).maybeSingle();
    ...
    /* b. Render React template to HTML ------------------------------------ */ addBreadcrumb('Rendering email template', 'template');
    ...
    const html = renderToStaticMarkup(React.createElement(CallSummaryEmail, {
      summary,
      phoneNumber,
      analysis,
    ...
    /* c. Send email via Brevo REST API --------------------------------- */ const apiKey = Deno.env.get('BREVO_API_KEY');
    ...
    const sentryError = error instanceof Error ? error : new Error('Unexpected error in send-email-summary');
