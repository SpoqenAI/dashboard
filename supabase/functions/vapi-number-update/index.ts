 ...
 */
async function getUserVAPIAssistantId(userId: string): Promise<string | null> {
...
/**
 * Update VAPI assistant with phone number configuration
 */
async function updateVAPIAssistant(
  ...
  try {
    ...
    console.log('VAPI assistant updated successfully:', result.id);
...
async function linkPhoneToVAPIAssistant(
  ...
  try {
    ...
    // Update VAPI assistant with retry logic
    const result = await retryWithBackoff(
      () => updateVAPIAssistant(assistantId, phoneNumber),
      3, // max retries
...
globalThis.Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      const result = await linkPhoneToVAPIAssistant(user_id, phone_number);
