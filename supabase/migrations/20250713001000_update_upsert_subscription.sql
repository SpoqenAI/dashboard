-- Migration: Update upsert_subscription for archiving, fallback, and user_id-centric logic
-- Purpose: Archive old subscriptions, automate fallback to free, and always use user_id for business logic
...
DECLARE
  ...
  v_is_cancel boolean := v_status IN ('canceled', 'deleted', 'past_due');
...
BEGIN
  ...
  -- 3. If this is a cancellation, insert a new free row if none exists
  IF v_is_cancel THEN
    SELECT EXISTS (
