-- =====================================================
-- Fix Security Vulnerability in create_full_profile Function
-- =====================================================
-- This migration updates the create_full_profile function to:
-- 1. Change from SECURITY DEFINER to SECURITY INVOKER to enforce RLS
-- 2. Add explicit authorization checks to prevent unauthorized access
-- 3. Ensure users can only create/modify their own profiles
...
-- Update function comment to reflect security changes
COMMENT ON FUNCTION public.create_full_profile(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) IS 
'Creates a complete user profile atomically within a transaction. Uses SECURITY INVOKER to enforce RLS policies. Users can only create/modify their own profiles. Upserts records in profiles, user_settings, and user_subscriptions tables. Returns JSON with success status and created records.';

-- Log the security fix
DO $$
BEGIN
    RAISE NOTICE 'Security fix applied: create_full_profile function updated to SECURITY INVOKER with authorization checks';
END $$;
