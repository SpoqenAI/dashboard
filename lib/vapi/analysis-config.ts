...
Extract the following information in the exact JSON format specified by the schema.`,
  structuredDataSchema: {
    ...
    properties: {
      callPurpose: {
        type: 'string',
        description:
          'Brief description of the main purpose or reason for the call',
      },
      sentiment: {
        type: 'string',
      ...
      keyPoints: {
        ...
        description:
          'Important topics or discussion points from the call (max 1-2 sentences each)',
      },
      followUpItems: {
        type: 'array',
        items: {
        ...
        description:
          'Specific actions that need to be taken as follow-up (actionable items only)',
      },
      urgentConcerns: {
        type: 'array',
        items: {
        ...
        description:
          'Time-sensitive or critical issues that require immediate attention (only if genuinely urgent)',
      },
    },
    required: [
      'callPurpose',
  ...
  successEvaluationPrompt:
    'You are an expert call evaluator for real estate agents. Determine if the call was successful based on whether the agent achieved their objectives (lead qualification, appointment setting, information gathering, etc.)',
