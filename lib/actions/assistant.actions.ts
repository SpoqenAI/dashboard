...
export async function provisionAssistant(userId: string): Promise<void> {
  ...
  if (fetchError || !assistant) {
    logger.info('ASSISTANT_PROVISIONING', 'No assistant found, creating default assistant', {
      userId: logger.maskUserId(userId),
    });
    ...
    logger.info('ASSISTANT_PROVISIONING', 'Successfully created default assistant', {
      userId: logger.maskUserId(userId),
  ...
  if (assistant.status === 'provisioning' || assistant.status === 'active') {
    logger.info('ASSISTANT_PROVISIONING', 'Idempotency check: Assistant already provisioning or active', {
      userId: logger.maskUserId(userId),
      assistantId: assistant.id,
  ...
  try {
    ...
    logger.info('ASSISTANT_PROVISIONING', 'Starting Twilio phone number provisioning', {
      assistantId: assistant.id,
    });
    ...
    if (isTestMode) {
      ...
      logger.info('ASSISTANT_PROVISIONING', 'Using Twilio test mode with magic phone number', {
        assistantId: assistant.id,
      });
      ...
      if (availableNumbers.length === 0) {
        throw new Error('No US voice-capable numbers are currently available from Twilio.');
      }
    ...
    logger.info('ASSISTANT_PROVISIONING', 'Successfully provisioned Twilio phone number', {
      assistantId: assistant.id,
    ...
    if (!vapiWebhookSecret) {
      throw new Error('Missing required VAPI_WEBHOOK_SECRET environment variable');
    }

    if (!appUrl) {
      throw new Error('Missing required NEXT_PUBLIC_APP_URL environment variable');
    }
    ...
    if (!vapiResponse.ok) {
      ...
      throw new Error(`Vapi API request failed: ${vapiResponse.status} ${errorText}`);
    }

    const vapiAssistant = await vapiResponse.json();

    logger.info('ASSISTANT_PROVISIONING', 'Successfully created Vapi assistant', {
      assistantId: assistant.id,
    ...
    const { error: phoneInsertError } = await supabase.from('phone_numbers').insert({
    ...
    if (phoneInsertError) {
      throw new Error(`Failed to insert phone number record: ${phoneInsertError.message}`);
    }

    // Update assistant to active status
    const { error: finalUpdateError } = await supabase.from('assistants').update({
      status: 'active',
    ...
    if (finalUpdateError) {
      throw new Error(`Failed to finalize assistant status: ${finalUpdateError.message}`);
    }

    logger.info('ASSISTANT_PROVISIONING', 'Provisioning workflow completed successfully', {
      assistantId: assistant.id,
      userId: logger.maskUserId(userId),
      phoneNumber: twilioNumber.phoneNumber,
      vapiAssistantId: vapiAssistant.id,
    ...
    await supabase
      ...
      .update({ 
        status: 'error', 
        status_detail: error.message || 'Unknown error occurred during provisioning' 
      })
      .eq('id', assistant.id);
